...
function InviteMemberDialog({ open, memberIds, onClose, onSearch, onInvite }: { open: boolean; memberIds: string[]; onClose: () => void; onSearch: (query: string) => Promise<Member[]>; onInvite: (id: string) => void }) {
  const [query, setQuery] = useState("");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      onSearch(query).then((results) => setMembers(results.filter((member) => !memberIds.includes(member.id)))).catch(() => setMembers([]));
    }, 250);
    return () => window.clearTimeout(timer);
  }, [memberIds, onSearch, query]);

  useEffect(() => {
    if (!open) { setQuery(""); setMembers([]); setSelected([]); }
  }, [open]);

  if (!open) return null;
  return <div className="invite-dialog-backdrop" onClick={onClose} role="presentation"><section className="invite-dialog" role="dialog" aria-modal="true" aria-labelledby="invite-member-title" onClick={(event) => event.stopPropagation()}><header><div><h2 id="invite-member-title">Üye davet et</h2><p>İsim veya telefon numarasıyla arayın.</p></div><IconButton label="Kapat" onClick={onClose}><X size={19} /></IconButton></header><label className="drawer-search"><Search size={18} /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="İsim veya telefon ara" /></label><div className="member-results selectable">{members.map((member) => <label key={member.id}><input type="checkbox" checked={selected.includes(member.id)} onChange={() => setSelected((current) => current.includes(member.id) ? current.filter((id) => id !== member.id) : [...current, member.id])} /><Avatar name={member.displayName} photoPath={member.photoPath} /><span><b>{member.displayName}</b><small>{member.phone}</small></span></label>)}</div>{query.trim() && !members.length ? <p className="invite-empty">Davet edilebilecek üye bulunamadı.</p> : null}<footer className="invite-dialog-actions"><span>{selected.length ? `${selected.length} kişi seçildi` : "Henüz üye seçilmedi"}</span><button className="primary-button" disabled={!selected.length} onClick={() => { selected.forEach(onInvite); onClose(); }}>Davet gönder</button></footer></section></div>;
}

function SenateDrawer({ open, conversation, user, onClose, onSave, onInvite, onPermission, onRemove, onLeave, onTransfer, onDelete }: { open: boolean; conversation: Conversation | null; user: User; onClose: () => void; onSave: (name: string, description: string, photo?: File) => void; onInvite: (id: string) => void; onPermission: (id: string, value: boolean) => void; onRemove: (id: string) => void; onLeave: () => void; onTransfer: (id: string) => void; onDelete: () => void }) {
  const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [photo, setPhoto] = useState<File | null>(null); const [inviteOpen, setInviteOpen] = useState(false); const photoRef = useRef<HTMLInputElement | null>(null);
  useEffect(() => { setName(conversation?.title ?? ""); setDescription(conversation?.description ?? ""); setPhoto(null); if (photoRef.current) photoRef.current.value = ""; }, [conversation]);
  if (!open || !conversation || conversation.type !== "senate") return null;
  return <><div className="drawer-backdrop" onClick={onClose}><aside className="side-drawer senate-drawer" onClick={(event) => event.stopPropagation()}><header><div><h2>Senato bilgileri</h2><p>{conversation.members.length} üye</p></div><IconButton label="Kapat" onClick={onClose}><X size={19} /></IconButton></header><div className="drawer-profile"><Avatar name={conversation.title} photoPath={conversation.photoPath} senate large /><div><strong>{conversation.title}</strong><span>{conversation.description || "Açıklama eklenmemiş."}</span></div></div>{conversation.canEdit ? <section className="drawer-form"><h3>Grup bilgileri</h3><input value={name} onChange={(event) => setName(event.target.value)} /><textarea value={description} onChange={(event) => setDescription(event.target.value)} /><label className="senate-photo-upload"><input ref={photoRef} hidden type="file" accept="image/*" onChange={(event) => setPhoto(event.target.files?.[0] ?? null)} /><span><Camera size={16} />Fotoğrafı değiştir</span><small>{photo ? photo.name : "Yeni fotoğraf seçilmedi"}</small></label><button className="secondary-button" onClick={() => onSave(name, description, photo ?? undefined)}>Değişiklikleri kaydet</button></section> : null}<section className="member-management"><header className="member-management-header"><h3>Üyeler</h3>{conversation.canInvite ? <button className="invite-member-button" onClick={() => setInviteOpen(true)} aria-label="Yeni üye davet et" title="Yeni üye davet et"><Plus size={17} /></button> : null}</header>{conversation.members.map((member) => <div key={member.id} className="member-row"><Avatar name={member.displayName} photoPath={member.photoPath} /><span><b>{member.displayName}</b><small>{member.id === conversation.createdById ? "Kurucu" : member.canInvite ? "Davet yetkisi var" : "Üye"}</small></span>{conversation.canEdit && member.id !== user.id ? <div className="member-actions"><button title="Davet yetkisi" onClick={() => onPermission(member.id, !member.canInvite)}><UserPlus size={16} /></button><button title="Kuruculuğu devret" onClick={() => { if (window.confirm(`${member.displayName} kurucu olsun mu?`)) onTransfer(member.id); }}><Shield size={16} /></button><button title="Üyeyi çıkar" onClick={() => { if (window.confirm(`${member.displayName} çıkarılsın mı?`)) onRemove(member.id); }}><UserMinus size={16} /></button></div> : null}</div>)}</section><section className="settings-rows danger-section">{!conversation.canEdit ? <button className="setting-row" onClick={onLeave}><span><ArrowLeft size={19} /><b>Senatodan ayrıl</b></span><ChevronRight size={18} /></button> : null}{conversation.canEdit ? <button className="setting-row danger-row" onClick={() => { if (window.confirm("Bu senato ve tüm mesajları kalıcı olarak silinecek.")) onDelete(); }}><span><Trash2 size={19} /><b>Senatoyu sil</b></span><ChevronRight size={18} /></button> : null}</section></aside></div><InviteMemberDialog open={inviteOpen} memberIds={conversation.members.map((member) => member.id)} onClose={() => setInviteOpen(false)} onSearch={async (query) => (await api.members(query)).members} onInvite={onInvite} /></>;
}
